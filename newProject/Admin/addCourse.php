...
if (isset($_REQUEST['courseSubmitBtn'])) {
    ...
    if (($_REQUEST['course_name'] == "") || ($_REQUEST['course_desc'] == "") || ($_REQUEST['course_author'] == "") || ($_REQUEST['course_duration'] == "") || ($_REQUEST['course_original_price'] == "") || ($_REQUEST['course_price'] == "")) {
        ...
        $course_img_temp = $_FILES['course_img']['temp_name'];
        $course_img_folder = '../images/' . $course_img;
        move_uploaded_file($course_img_temp, $course_img_folder);
        // move_uploaded_file($course_img_temp, '../images/'.$course_img);

        // $course_name = preg_replace("/\s+/", "-", $course_name);
        // $picname = pathinfo($course_name, PATHINFO_FILENAME);
        // $picext = pathinfo($course_name, PATHINFO_EXTENSION);

        // $mix = $picname . date("YmjHis") . $picext;

        // move_uploaded_file($course_img_temp, "../images/" . $mix);

        $sql = "INSERT INTO course (course_name, course_desc, course_author, course_img, course_duration, course_price, course_original_price) VALUES ('$course_name', '$course_desc', '$course_author', '$course_img_temp', '$course_duration', '$course_price', '$course_original_price')";
